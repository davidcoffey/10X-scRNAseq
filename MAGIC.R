...
data_MAGIC <- magic(data, knn=15, genes=c("CD8a", "GZMB", "PDCD1"))
data_MAGIC <- magic(data, genes="all_genes", 
                    knn=15, init=data_MAGIC)
write.csv(data_MAGIC, file=paste(path, sep = "", "MAGIC_transformed_matrix.csv"))

ggplot(data) +
  geom_point(aes(CD8a, GZMB, color=PDCD1)) +
  scale_color_viridis(option="B")
ggsave(paste(path, sep = "", "EMT_data_R_before_magic.png"), width=5, height=5)

ggplot(data_MAGIC) +
  geom_point(aes(CD8a, GZMB, color=PDCD1)) +
  scale_color_viridis(option="B")
ggsave(paste(path, sep = "", "EMT_data_R_after_magic.png"), width=5, height=5)
...
ggplot(data_MAGIC_PCA) +
  ...
  scale_color_viridis(option="B") +
  labs(color="PDCD1")
ggsave(paste(path, sep = "", "EMT_data_R_pca_colored_by_magic.png"), width=5, height=5)
...
ggplot(data_PHATE) +
  ...
  scale_color_viridis(option="B") +
  labs(color="PDCD1")
ggsave(paste(path, sep = "", "EMT_data_R_PHATE_colored_by_magic.png"), width=5, height=5)
